...
class UCDIR(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K_A=65536, K_B=65536,
        ...
        #base_encoder  使用的是resnet50 输出的特征维度是128维
        self.K_A = K_A
        self.K_B = K_B
        self.m = m
        self.T = T

        self.selfentro_temp = selfentro_temp #selfentropy的临时变量
        self.num_cluster = num_cluster #聚类数量
        self.cwcon_filterthresh = cwcon_filterthresh #cw loss的阈值限制

        norm_layer = partial(SplitBatchNorm, num_splits=2) #partial 函数的功能就是：把一个函数的某些参数给固定住，返回一个新的函数。

        self.encoder_q = base_encoder(num_classes=dim)
        self.encoder_k = base_encoder(num_classes=dim, norm_layer=norm_layer)

        self.alpha = 1.0  #用于哈希码的生成
        ...
        if mlp:  # hack: brute-force replacement
            ...
            # self.encoder_q.fc = nn.Sequential(nn.Linear(dim_mlp, dim))
            # self.encoder_k.fc = nn.Sequential(nn.Linear(dim_mlp, dim))
    ...
    def forward(self, im_q_A, im_q_B, im_k_A=None, im_id_A=None,
        ...
        if is_eval: # 若是验证，我们只是用动量编码器的特征
            k = self.encoder_k(im_q)
            k = F.normalize(k, dim=1)
            # k = torch.tanh(self.alpha * k)  #改为二值检索
        ...
        # q = F.normalize(q, dim=1)  #原模型为实值检索
        q = torch.tanh(self.alpha * q)  #改为二值检索
        ...
        # 当存在聚类中心时，启用loss_cw(Lcw)  loss_selfentro(Lse) loss_distlogit(Ldd) 三组损失
