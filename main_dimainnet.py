...
parser.add_argument('--data-A', default='G:/ucdir_dataset/DomainNet/clipart_', metavar='DIR Domain A', help='path to domain A dataset')# 更改参数
parser.add_argument('--data-B', default='G:/ucdir_dataset/DomainNet/sketch_', metavar='DIR Domain B', help='path to domain B dataset')# 更改参数
...
# parser.add_argument('--eval-epochs', default=20, type=int, metavar='N',
#                     help='number of total epochs to run')
parser.add_argument('-b', '--batch-size', default=64, type=int,
                    metavar='N',
                    help='mini-batch size (default: 256)') # 更改参数
...
parser.add_argument('--clean-model', default='./scripts/moco_v2_800ep_pretrain.pth.tar', type=str, metavar='PATH',
                    help='path to clean model (default: none)')# 更改参数
...
parser.add_argument('--moco-m', default=0.9991, type=float,
...
parser.add_argument('--exp-dir', default='ablation_09991_domainnet_clipart_-sketch_100', type=str,
                    help='the directory of the experiment')#更改参数
parser.add_argument('--ckpt-save', default=5, type=int,
                    help='the frequency of saving ckpt')
parser.add_argument('--num-cluster', default='7', type=str,
                    help='number of clusters for self entropy loss')#更改参数
...
parser.add_argument('--cwcon-startepoch', default=10, type=int,
                    help='the start epoch for scluster-wise contrastive loss')
parser.add_argument('--cwcon-satureepoch', default=50, type=int,
...
parser.add_argument('--selfentro-temp', default=0.1, type=float,
                    help='the temperature for self-entropy loss')
parser.add_argument('--selfentro-startepoch', default=50, type=int,
                    help='the start epoch for self entropy loss')
parser.add_argument('--selfentro-weight', default=0.5, type=float,
                    help='the start weight for self entropy loss')
parser.add_argument('--distofdist-startepoch', default=50, type=int,
                    help='the start epoch for dist of dist loss')
parser.add_argument('--distofdist-weight', default=0.1, type=float,
                    help='the start weight for dist of dist loss')
parser.add_argument('--prec-nums', default='50,100,200', type=str,
                    help='the evaluation metric')#更改参数
...
def main_worker(gpu, args):
    ...
    if args.clean_model:# 这一段 用于加载预训练模型。
    ...
    for epoch in range(args.epochs):
        # 这句命令是为了初始化 features_A, features_B
        ...
        info_save.write(json.dumps(args.__dict__))
        #先进行训练，encoder 会反向传播， momentum_encoder 不会反向传播
        print("----------------------------------Training--------------------------------------")
        ...
        print("----------------------------------Evaluating--------------------------------------")
        #在验证时，计算特征直接使用了momentum_encoder,而不用encoder
...
def train(train_loader, model, criterion, optimizer, epoch, args, info_save, cluster_result):
    ...
    for i, (images_A, image_ids_A, images_B, image_ids_B, cates_A, cates_B) in enumerate(train_loader):
        #  images_A : [q_A, k_A], images_B : [q_B, k_B],  q k  为同一张图片的 3*224*224特征
        #  image_ids_A : index_A,  image_ids_B : index_B,  索引信息
        #  cates_A : target_A, cates_B : target_B  类别信息
        ...
        #当训练轮数达到 warmup_epoch 数值(默认20)  加入 loss_cwcon 损失。
        if epoch >= args.warmup_epoch:
        ...
        # 当训练轮数达到 selfentro_startepoch 数值(默认100)  加入 losses_selfentro 损失。
        ...
        # 当训练轮数达到 distofdist_startepoch 数值(默认100)  加入 losses_distlogits损失。
